...
|---|---|---|---|---|
| JTBD-1 | preparing a startup for a first SOC 2 readiness review | to see, within one session, which SOC 2 Trust Services Criteria clauses I satisfy and which are gaps (with the underlying NIST 800-53 evidence per ADR-0013) | prioritize engineering work and stop guessing at coverage | Founding engineer |
...
| NG-2 | **Not a CPA firm.** AuditPilot produces internal readiness assessments and draft outputs for human review. It cannot produce draft attestations, draft certifications, or any form of draft SOC 2 report that a licensed CPA firm would issue under SSAE No. 18 / AT-C 205. Only licensed CPA firms can produce SOC 2 readiness opinions. | AICPA UPAct (Uniform Practice Act) imposes civil and criminal liability for unlicensed practice of accountancy. This is a hard legal constraint, not a stylistic preference. |
...
| NG-5 | **Not a multi-framework GRC platform in v1.** Version 1 covers the SOC 2 Trust Services Criteria, grounded in NIST SP 800-53 Rev 5 as the underlying public-domain control catalog (ADR-0013). ISO 27001, HIPAA, PCI-DSS, and GDPR mappings are deferred to v2 even though the catalog has clean precedent for those framings. | Breadth before depth is the wrong tradeoff for a reference architecture. One framework done well is more valuable to engineers than six frameworks done poorly. |
...
| **Automated control evidence collection** | AuditOrchestrator reads GitHub evidence, maps it to NIST 800-53 Rev 5 controls via `compliance-kb-mcp`, and surfaces which SOC 2 Trust Services Criteria are satisfied via the curated TSC mappings (ADR-0013). Produces a structured control map with gap flags. | US-003, US-004 |
| **Control posture grid** | Dashboard component showing the SOC 2 Trust Services Criteria clauses (Common Criteria + Availability + Confidentiality + Processing Integrity + Privacy) as green / yellow / red, updated in real time as the orchestrator streams results. Each TSC clause expands into the underlying NIST 800-53 controls that satisfy it (per ADR-0013). | US-006 |
