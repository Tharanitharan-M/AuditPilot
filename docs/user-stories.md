...
- Frontend: `apps/web/app/(auth)/sign-up/page.tsx` and `apps/web/app/(auth)/sign-in/page.tsx`. Clerk components and middleware.
- Backend: `apps/api` verifies JWT on every authenticated request via `Depends(current_user)`.
- ADR-0008 (Clerk selection); SRS FR-001 through FR-006.
- Data dependency: `users` table populated on first sign-in via Clerk webhook to `POST /api/internal/users/upsert`.

### PLAN.md chunks generated by this story

- [ ] Chunk 3.2 — Clerk signup page
- [ ] Chunk 3.3 — Clerk login page
...
- Backend: `DELETE /api/connectors/{connector_id}`. Calls Clerk auth API endpoint to revoke OAuth grant; deletes `connectors` row.
...
- [ ] All three acceptance criteria pass
- [ ] Auto test: Pytest verifies revoke is called on the Clerk auth client
...
WHEN I click "Approve and download"
THEN the frontend transparently refreshes the JWT via the Clerk session refresh flow
